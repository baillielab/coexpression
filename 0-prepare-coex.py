...
import string, copy, stat, random, math, itertools, operator
import subprocess
...
parser.add_argument('-p', '--permutationmode',	type=str, choices=['circular', 'post', 'backcirc', 'background'], default='circular',	help='permutation mode')
...
parser.add_argument('-m', '--selectionthreshold_is_j',	action="store_true", default=False,	help='match selectionthreshold_is_j')
...
if permutationmode == 'post': output_directory_label += "_POST"
elif permutationmode == 'backcirc': output_directory_label += "_BACKCIRC"
elif permutationmode == 'circular': output_directory_label += "_CIRCULAR"
elif permutationmode == 'background': output_directory_label += "_BACKGROUND"
...
if not os.path.exists(sofile): 
	cmd = "gcc -shared  -O3 -fPIC -fopenmp %scoexpression_v2.c -o %scoexpression_v2.so"%(sourcefilesdir, sourcefilesdir)
...
#-------------------------------------------------------------------------------
runcommand = "python \
0-run-coex.py -y %s -f %s -n %s -p %s -s %s -e %s -t %s -x %s -q %s -w %s"%(version, snp_details_file, numperms, permutationmode, nsp, useremail, numthreads, specialexpressionfile, specialbedfile, window)
...
if numhits >1: #CAN ONLY PROCEED IF THERE ARE ENOUGH SNPS TO BE WORTH MAPPING
	...
	for shiftindex in range(len(shifts)):
		...
		if os.path.exists(mapfiles[shift]) and shift != 0:
			if doitagain:
				if verbose: print ("permutation file", mapfiles[shift], "exists but will not be used as doitagain is True")
			else:
		...
		if permutationmode == 'circular' or shift == 0: 
			...
			elif verbose: print ("CIRCULAR randomising shift = ", shift, permutationmode)
			for chrom in snp_bed_data:
				for address in snp_bed_data[chrom]:
...
if len(bonuspromoters) > 0:
	...
	if permutationmode != 'post':
		...
		if verbose: print("Permutations reset to post-mapping - additional promoters added manually so pre-mapping permutations are not applicable")
...
if permutationmode == 'post':
	...
	#-------------------------------------#
	real_to_join = join_nearby(primary_dict.keys(), prom_addresses, globalcorrelationlist, exp_dict, pval_to_join, correlationmeasure, soft_separation, anticorrelation)
	realdist = sorted([len(real_to_join[x]) for x in real_to_join])

	if verbose: print ("***", len(primary_dict.keys()), "promoters hit, in",len(real_to_join),"distinct regions in the input dataset ***")
	...
	for i in range(numperms): #add some space for wastage from failed perms
		...
		if permutationmode == 'postcirc':
					...
					if runcount == 0: # first run
						...
						for n in needed:
							...
							t=[all_labels[randindex + x] for x in range(r)] #transient
 							tg = join_nearby(t, prom_addresses, globalcorrelationlist, exp_dict, pval_to_join, correlationmeasure, soft_separation, anticorrelation)
 							b=[x for x in tg if len(tg[x]) >= n]
 							if len(b)>0:
								newlist+=tg[random.choice(b)][:n] #add in n adjacent promoters, chosen at random
					newgroups = join_nearby(newlist, prom_addresses, globalcorrelationlist, exp_dict, pval_to_join, correlationmeasure, soft_separation, anticorrelation)
					for new in newgroups:
						if len(newgroups[new]) in needed:
							thisperm += newgroups[new]
							needed.remove(len(newgroups[new]))
					perm_to_join = join_nearby(thisperm, prom_addresses, globalcorrelationlist, exp_dict, pval_to_join, correlationmeasure, soft_separation, anticorrelation)
			...
			if notready: # the perfect distribution was not found despite many searches. Now choose the right distribution from the permuted excess:
				...
				for desiredgrouplen in list(set(real)):
					...
					while not finished:
						...
						if verbose: print ("searchpool lengths = ", [len(perm_to_join[x]) for x in remaining if abs(len(perm_to_join[x])-desiredgrouplen) <= acceptable_difference])
			...
			if verbose: print (len(primary_dict.keys()), "promoters hit, in",len(real_to_join),"distinct regions in input dataset")
			if verbose: print (len(thisperm), "promoters hit, in",len(perm_to_join),"distinct regions for this permutation")
			#---------------------------------------------
		o=open(permfile,"w")
